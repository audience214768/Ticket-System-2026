...
class BPlusTree {
 ...
 public:
  explicit BPlusTree(
    ...
    shared_ptr<BufferPoolManager> buffer_pool_manager, 
    int leaf_max_size = 0, 
...
auto BPLUSTREE_TYPE::FindLeafOpt(const KeyType &key, Context &ctx, bool is_get) -> page_id_t {
    //std::cerr << header_page_id_ << std::endl;
    ...
    //std::cerr << "root size " << page.As<BPlusTreePage>()->GetSize() << std::endl;
    while (!page.As<BPlusTreePage>()->IsLeafPage()) {
      ...
      if (is_get && memcmp(internal_page->KeyAt(index + 1).fixed_key.key, key.fixed_key.key, key.LEN) != 0) {
        ctx.read_set_.clear();
        ctx.read_header_page_.Drop();
      }
...
void BPLUSTREE_TYPE::GetValue(const KeyType &key, vector<ValueType> *result) {
  ...
  int i = index + 1;
  while (true) {
    for (; i < leaf_page->GetSize() && /*leaf_page->KeyAt(i).GetKey() == key.GetKey()*/memcmp(leaf_page->KeyAt(i).fixed_key.key, key.fixed_key.key, key.LEN) == 0; i++) {
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value) -> bool {
  //std::cerr << "insert" << std::endl;
  {
    ...
    //std::cerr << ctx.root_page_id_ << std::endl;
    if (ctx.root_page_id_ != INVALID_PAGE_ID) {
  ...
  //std::cerr << "check" << std::endl;
  //std::cerr << leaf_page_id << std::endl;
  WritePageGuard leaf_page_guard = bpm_->WritePage(leaf_page_id);
  LeafPage *leaf_page = leaf_page_guard.AsMut<LeafPage>();
  ...
  if(guard.GetPageId() == ctx.root_page_id_) {
    if(guard.As<BPlusTreePage>()->IsLeafPage()) {
      ...
      if(root_page->GetSize() == 0) {
        ...
        header_page->root_page_id_ = INVALID_PAGE_ID;
        //std::cerr << "empty tree now" << std::endl;
        guard.Drop();
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::Update(const KeyType &key, const ValueType &value) -> bool {
  Context ctx;
  auto leaf_page_id = FindLeafPess(key, ctx);
  if (ctx.root_page_id_ == INVALID_PAGE_ID) {
    return false;
  ...
  LeafPage *leaf_page = leaf_page_guard.AsMut<LeafPage>();
  auto index = leaf_page->Search(key, compare_);
  if (index < 0 || compare_(key, leaf_page->KeyAt(index)) != 0) {
    return false;
  }
