...
DiskManager::DiskManager(size_t file_id, const string &db_file_name):fileID_(file_id), db_file_name_(db_file_name) {
  ...
  if (!db_file_io_.is_open()) {
    db_file_io_.open(db_file_name_, ios::out | ios::binary);
    db_file_io_.close();
    db_file_io_.open(db_file_name_, ios::in | ios::out | ios::binary);
  } else {
    db_file_io_.seekg(0);
    ...
    db_file_io_.read(reinterpret_cast<char *>(&file_size_), sizeof(page_id_t));
  }
...
DiskManager::~DiskManager() {
  ...
  db_file_io_.write(reinterpret_cast<const char *>(&file_size_), sizeof(page_id_t));
  db_file_io_.close();
  log_file_io_.close();
}

void DiskManager::ReadPage(page_id_t page_id, char *data) {
  db_file_io_.seekg(OFFSET(page_id % DISK_PAGE_SIZE));
