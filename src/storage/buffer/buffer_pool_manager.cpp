...
void BufferPoolManager::UseFrame(frame_id_t frame_id, page_id_t page_id, bool is_write, unique_lock<mutex> &lock) {
  //std::cerr << page_id << " " << frame_id << std::endl;
  ...
  Access(frame_id);
  {
    ...
    //std::cerr << *reinterpret_cast<size_t *>(frame_info_[frame_id]->GetDataMut()) << std::endl;
    lock.unlock();
    //std::cerr << "check" << std::endl;
    future.get();
    lock.lock();
...
void BufferPoolManager::Evict(frame_id_t frame_id, page_id_t page_id, bool is_write, unique_lock<mutex> &lock) {
  ...
  frame_info_[frame_id]->is_loading_ = true;
  if (old_state) {
    //log_manager_->AppendLog(old_page_id, frame_info_[frame_id]->GetDataMut());
    //log_manager_->FlushToLsn(frame_info_[frame_id]->lsn_);
    promise<page_id_t> promise;
    auto future = promise.get_future();
    disk_scheduler_->Scheduler(DiskRequest{
...
void BufferPoolManager::Access(frame_id_t frame_id) {
  //std::cerr << "access " << frame_id << std::endl;
...
auto BufferPoolManager::WritePage(page_id_t page_id) -> WritePageGuard {
  ...
  if(frame_id == INVALID_FRAME_ID) {
    if(free_list_.empty()) {
      ...
      //std::cerr << evict_frame_id << " " << page_id << std::endl;
    ...
    free_list_.pop_back();
    //std::cerr << free_frame << std::endl;
    UseFrame(free_frame, page_id, true, lock);
    //std::cerr << free_frame << std::endl;
    //std::cerr << *reinterpret_cast<size_t *>(frame_info_[free_frame]->GetDataMut()) << std::endl;
    lock.unlock();
...
auto BufferPoolManager::ReadPage(page_id_t page_id) -> ReadPageGuard {
  ...
  if (frame_info_[frame_id]->is_loading_) {
    //std::cerr << frame_info_[frame_id]->is_loading_ << std::endl;
    frame_info_[frame_id]->cv_.wait(lock, [&]{ return frame_info_[frame_id]->is_loading_ == false; });
  }
  //std::cerr << frame_id << std::endl;
  Access(frame_id);
  lock.unlock();
  //std::cerr << frame_info_[frame_id]->pin_count_ << std::endl;
...
void BufferPoolManager::ClearAll() {
  unique_lock<mutex> lock(*bpm_mutex_);
  ...
  replacer_ = make_shared<Replacer>(frame_num_);
  for (size_t i = 0; i < disk_manager_.size(); i++) {
    disk_manager_[i]->Clear();
  }
}
...
BufferPoolManager::~BufferPoolManager() {
  for (int i = 0; i < frame_num_; i++) {
    ...
    if(frame_info_[i]->page_id_ != INVALID_PAGE_ID && frame_info_[i]->is_dirty_) {
      //std::cerr << "write " << frame_info_[i]->page_id_ << std::endl;
      //log_manager_->AppendLog(frame_info_[i]->page_id_, frame_info_[i]->GetDataMut());
      promise<page_id_t> promise;
      auto future = promise.get_future();
      disk_scheduler_->Scheduler(DiskRequest{
        .type = RequestType::kWrite,
